...
このテーマをシャドウ/オーバーライドするには、ファイルを`src/@rocketseat/gatsby-theme-docs`フォルダーに配置する必要があります。

シャドウイングとは何かわからない場合、またはシャドウイングについて質問がある場合は、Gatsby のドキュメントから[このチュートリアル](https://www.gatsbyjs.com/docs/themes/shadowing/)を確認してください。

## ロゴの変更

ロゴを変更したい場合は、次のファイルを作成してください。
...
でファイルを作成して`src/@rocketseat/gatsby-theme-docs/text/index.mdx`、テキストを編集します。例：
...
テーマには Emotion を使用します。テーマの色を変更するには、の下にファイルを作成します`src/@rocketseat/gatsby-theme-docs/styles/theme.js`。

アップデートを確認するには、サーバーを再起動する必要があることに注意してください。
...
## ReactLive スコープへの要素の追加

デフォルトでは、React LiveScope にいくつかの要素を追加します。さらに追加する必要がある場合は、次のファイルをシャドウイングするだけです:
...
スコープについて詳しく知りたい場合は、[この説明を確認](https://github.com/FormidableLabs/react-live#how-does-the-scope-work)してください。
